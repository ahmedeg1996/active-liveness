...
def challenge_result(question, out_model,blinks_up):
    """
    check if frame fall or pass the task

    Parameters
        question : str 
            task that want to check ,if client do
        
        out_model : dict()
            contain key and value key is the task value is the result of task
        
        blinks_up : int  (0 or 1)
            to check if user blink eye or not
    
    Returns 
        str : ("pass" , "fall")
        return if user pass the the task or fal
