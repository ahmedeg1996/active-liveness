...
def detect_liveness(im,COUNTER=0,TOTAL=0):
    """
    Detect liveness if the person do the task 

    Parameters
        ----------
        im : array int(  image  (RGB ) )
             RGB Image it come from webcam

        COUNTER : int 
            Counter is the over all count blinks consecutives
        TOTAL  : int 
             Total is the over all  blinks       

    Returns 

        -  Dictionary {key: value}
            key is the task
            values  the result of task
    """
    ...
    if len(boxes_face)!=0:
        # extract face which has max area
