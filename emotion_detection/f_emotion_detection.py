...
class predict_emotions():
    ...
    def preprocess_img(self,face_image,rgb=True,w=48,h=48):
        ...
        Parameter :
            face_image : array (int)
                croped face for image

            rgb : boolean   
                if image is rgb   or  gray

            w , h : int
                is the height and width dimension which image will resize 
        Return
             
            Face image with resize (w,h,1)
    ...
    def get_emotion(self,img,boxes_face):
        ...
        if len(boxes_face)!=0:
            ...
            emotions = []
            boxes_face = []
        # emotion for the person and croped face
